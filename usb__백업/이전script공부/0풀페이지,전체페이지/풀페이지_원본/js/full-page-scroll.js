...
(function () {
	...
	fullScroll.prototype.buildDots = function () {		
		...
		// 수정
		document.querySelector(".menuWrap").children[0].querySelector("a").setAttribute("class","active")
	...
	fullScroll.prototype.buildPublicFunctions = function () {
		...
		this.animateScroll = function () {
			...
			for (var i = 0; i < this.ul.childNodes.length; i++) {
					...
					//수정
					document.querySelector(".menuWrap").children[i].querySelector("a").setAttribute("class","")
					if (i == this.defaults.currentPosition) {
					this.ul.childNodes[i].firstChild.classList.add('active');		
					//수정
					document.querySelector(".menuWrap").children[i].querySelector("a").setAttribute("class","active")
				}
			}
		};
